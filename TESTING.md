...
| `string_builder.c` | `cn_sb_make/_append_char/_append_str/_prepend_str/_append_format/_to_str/_clear/_reverse/_free`. Stack backed storage (`CN_SB_STACK_STORAGE_CAP`), spill from stack to heap on overflow with content preserved, heap backed construction from the start, post-free state. Prepending onto an empty builder, in front of existing contents, mixed with appends, of an empty string, and spilling to the heap. |
...
| `type_stringify.c` | `cn_type_stringify` with and without an identifier. Primitives, pointers, pointer-to-pointer and qualified types; arrays including unsized, arrays of pointers and pointers to arrays; multi dimensional arrays; functions, `void` parameter lists and function pointers. Declarator shapes where the suffix ordering matters: array of function pointers, array of pointers to array, and a function returning a pointer to array. Buffer too small returning an empty string. |
